...
func (db *DB) CreateSession(input CreateSessionInput) (*AgentSession, error) {
	...
	_, err := db.conn.Exec(`
		INSERT INTO agent_sessions (id, task_id, provider, provider_session_id, status, tmux_window, tmux_pane, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
	`, id, input.TaskID, input.Provider, NullString(input.ProviderSessionID), SessionStatusIdle, NullString(input.TmuxWindow), NullString(input.TmuxPane), now, now)
	if err != nil {
...
func (db *DB) GetSession(id string) (*AgentSession, error) {
	row := db.conn.QueryRow(`
		SELECT id, task_id, provider, provider_session_id, status, tmux_window, tmux_pane, log_file, created_at, updated_at
...
func scanSession(row *sql.Row) (*AgentSession, error) {
	...
	err := row.Scan(
		&s.ID, &s.TaskID, &s.Provider, &providerSessionID, &s.Status,
		&tmuxWindow, &tmuxPane, &logFile, &s.CreatedAt, &s.UpdatedAt,
	)
	if err != nil {
		return nil, err
	}
