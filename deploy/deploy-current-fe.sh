#!/usr/bin/env bash
# Deploy the current local frontend working tree to the remote server without restarting Codeburg.
#
# Usage:
#   ./deploy/deploy-current-fe.sh [ssh-host] [target-dir]
#
# Defaults:
...
echo "==> Deploying current frontend tree to ${SSH_HOST}:${TARGET_DIR}"
echo "==> Source commit: $(git rev-parse --short HEAD)"

LOCAL_ARCHIVE="$(mktemp /tmp/codeburg-current-fe.XXXXXX.tar)"
REMOTE_ARCHIVE="/tmp/codeburg-current-fe-archive-$$.tar"
...
LOCK_FILE="/tmp/codeburg-deploy-current-fe.lock"
...
exec 9>"$LOCK_FILE"
if ! flock -n 9; then
    echo "==> Another current frontend deploy is already running (lock: ${LOCK_FILE})"
    exit 1
fi

STAGING_DIR="$(mktemp -d /tmp/codeburg-current-fe-deploy.XXXXXX)"
cleanup() {
    rm -rf "$STAGING_DIR"
    rm -f "$ARCHIVE_PATH"
}
...
echo "==> Current frontend deploy complete (no restart performed)."
REMOTE
